...
namespace MaxyGames.UNode.Editors {
	public class RealtimePreviewSourceWindow : PreviewSourceWindow {
		...
		protected override void OnEnable() {
			...
			uNodeEditor.onChanged += PrepareGenerate;
			Undo.undoRedoPerformed -= PrepareGenerate;
			Undo.undoRedoPerformed += PrepareGenerate;
			PrepareGenerate();
		}
		...
		private void OnFocus() {
			uNodeEditor editor = uNodeEditor.window;
			if(editor != null) {
				if(currGO != editor.selectedTab.owner) {
					DoGenerate(editor.selectedTab.owner);
				}
			}
		}
				...
				lock(lockThread) {
					...
					uNodeThreadUtility.Queue(() => {
						...
						if(sourceIsHighligted && prevHeighlight == s) {
							...
							Repaint();
							uNodeThreadUtility.Queue(() => {
								if(uNodeEditor.window != null && object.ReferenceEquals(uNodeEditor.window.graphData?.RootOwner, obj)) {
									OnChanged(uNodeEditor.window.graphData);
								}
							});
						}
						if(script.hasError) {
