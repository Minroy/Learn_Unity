...
namespace MaxyGames.UNode.Editors {
	...
	internal class BookmarkTreeView : TView {
		private static readonly string DataKey = "UNODE_BOOKMARK";

		class BookmarkTreeViewItem : TViewItem {
		...
		public BookmarkTreeView(MultiColumnHeader multiColumnHeader = null) : base(new TViewState(), multiColumnHeader) {
			showBorder = true;
		...
		protected override bool CanBeParent(TViewItem item) {
		...
		protected override bool CanRename(TViewItem item) {
			return false;
		}
		...
		public TViewItem FindItem(int id) {
			return FindItem(id, rootItem);
		}

		protected override Rect GetRenameRect(Rect rowRect, int row, TViewItem item) {
		...
		protected override TViewItem BuildRoot() {
			...
			if(root.children == null) {
				root.children = new List<TViewItem>();
			}
		...
		void RecursiveBuildTree(TViewItem parent, Bookmark bookmark) {
				...
				if(Event.current.type == EventType.Repaint) {
					...
					if(tree.bookmark is BookmarkCanvas canvas) {
						...
						if(reference != null) {
							...
							var width = GraphEditorUtility.ReferenceTree.DrawReferencePath(args.rowRect, paths);
