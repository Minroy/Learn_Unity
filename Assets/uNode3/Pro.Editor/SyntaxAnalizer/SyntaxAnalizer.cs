...
namespace MaxyGames.UNode.Editors {
	public static class SyntaxAnalizer {
		public static string AnalizeCode(string source) {
			if(uNodePreference.preferenceData.generatorData.analyzeScript) {
				if(uNodePreference.preferenceData.generatorData.removeUnnecessaryCode) {
					source = RemoveUnnecessaryCode(source);
				}
				if(uNodePreference.preferenceData.generatorData.formatScript) {
					source = FormatCode(source);
				}
				if(uNodePreference.preferenceData.generatorData.preferExtensionMethodSyntax) {
					var root = RoslynUtility.GetSyntaxTree(source, out var model);
					var rewriter = new ExtensionRewriter(model);
		...
		public static string RemoveUnnecessaryCode(string source) {
			SyntaxNode root = RoslynUtility.GetSyntaxTree(source, out var model);

			var rewriter = new RemoveThisQualifierRewriter(model);
	...
	static class ParenthesizedExpressionSyntaxExtensions {
		public static bool CanRemoveParentheses(this ParenthesizedExpressionSyntax node, SemanticModel semanticModel) {
			...
			if(expression.IsKind(SyntaxKind.ParenthesizedExpression) ||
				...
				if(previousChar == '!' && nextChar == '(') {
					if(expression is ParenthesizedExpressionSyntax innerExpression) {
						if(innerExpression.Expression.IsKind(SyntaxKind.ParenthesizedExpression)) {
							// In cases like: !((x)) -> !(x), we can remove parents.
							return true;
						}
						if(innerExpression.Expression is BinaryExpressionSyntax) {
							return false;
						}
						//UnityEngine.Debug.Log(innerExpression.Expression.GetType());
					}
					//// Don't remove parens in cases like: !(x) -> !x, as that can change the meaning of the code.
					//return false;
				}
				return true;
			}
	...
	class RemoveThisQualifierRewriter : CSharpSyntaxRewriter {
		private readonly SemanticModel _semanticModel;

		public RemoveThisQualifierRewriter(SemanticModel semanticModel) {
			_semanticModel = semanticModel;
		}

		public override SyntaxNode VisitMemberAccessExpression(MemberAccessExpressionSyntax node) {
			// Only handle `this.someMember`
			...
			var identifierName = node.Name.Identifier.Text;

			// Check if a local variable or parameter in scope shadows this name
			if(IsNameShadowed(node, identifierName, memberSymbol))
				return base.VisitMemberAccessExpression(node); // Keep "this."

			return node.Name.WithTriviaFrom(node); // Remove "this."
		}

		private bool IsNameShadowed(SyntaxNode accessNode, string name, ISymbol targetSymbol) {
			var current = accessNode;
			...
			while(current != null) {
				switch(current) {
					...
					case LambdaExpressionSyntax lambda:
						...
						if(lambdaParams.Any(p => p.Identifier.Text == name))
							return true;
						break;

					case BlockSyntax block:
						// Only look for locals declared before this node
						foreach(var statement in block.Statements) {
							if(statement.SpanStart >= accessNode.SpanStart)
								break;

							var locals = statement.DescendantNodes().OfType<VariableDeclaratorSyntax>();
							if(locals.Any(v => v.Identifier.Text == name))
								return true;
						}
						break;
				}
	...
	class ExtensionRewriter : CSharpSyntaxRewriter {
		private readonly SemanticModel _semanticModel;

		public ExtensionRewriter(SemanticModel semanticModel) {
			_semanticModel = semanticModel;
		}

		public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node) {
			var originalSymbol = _semanticModel.GetSymbolInfo(node).Symbol as IMethodSymbol;
			if(originalSymbol == null || !originalSymbol.IsExtensionMethod || originalSymbol.ReducedFrom != null)
			...
			var instance = arguments[0].Expression;
			var remainingArguments = SyntaxFactory.SeparatedList(arguments.Skip(1));

			var newMemberAccess = SyntaxFactory.MemberAccessExpression(
				SyntaxKind.SimpleMemberAccessExpression,
				instance,
				SyntaxFactory.IdentifierName(originalSymbol.Name));

			var newInvocation = SyntaxFactory.InvocationExpression(
				newMemberAccess,
				SyntaxFactory.ArgumentList(remainingArguments));

			var speculative = _semanticModel.GetSpeculativeSymbolInfo(
				node.SpanStart,
				newInvocation,
				SpeculativeBindingOption.BindAsExpression);

			if(speculative.CandidateReason != CandidateReason.None) {
				// Ambiguous or invalid binding
				return base.VisitInvocationExpression(node);
			}

			var rewrittenSymbol = speculative.Symbol as IMethodSymbol;

			if(rewrittenSymbol == null)
				return base.VisitInvocationExpression(node);

			// This comparison allows reduced extension form
			var isSame = SymbolEqualityComparer.Default.Equals(originalSymbol.OriginalDefinition, rewrittenSymbol.OriginalDefinition)
					  || SymbolEqualityComparer.Default.Equals(originalSymbol.OriginalDefinition, rewrittenSymbol.ReducedFrom?.OriginalDefinition);
