...
namespace MaxyGames.UNode.Editors {
		...
		public override bool StatementHandler(StatementSyntax syntax, ParserData data, out ParserResult result) {
			if(syntax is ForEachStatementSyntax && data.CanCreateNode) {
				...
				if(n.collection.isAssigned && n.collection.UseDefaultValue) {
					n.collection.ConnectTo(CSharpParser.ToValueNode(new(n.collection.DefaultValue), null, new ParserSetting(data)));
				}
	...
	public class ForeachVariableStatementHandles : SyntaxHandles {
		public override bool StatementHandler(StatementSyntax syntax, ParserData data, out ParserResult result) {
			if(syntax is ForEachVariableStatementSyntax && data.CanCreateNode) {
				var ex = syntax as ForEachVariableStatementSyntax;
				var n = CSharpParser.CreateNode<ForeachLoop>("foreach", data.parent);
				n.deconstructValue = true;
				n.collection.ConnectTo(CSharpParser.GetMemberFromExpression(ex.Expression, data.model, data.parent));
				if(n.collection.isAssigned && n.collection.UseDefaultValue) {
					n.collection.ConnectTo(CSharpParser.ToValueNode(new(n.collection.DefaultValue), null, new ParserSetting(data)));
				}
				...
				if(ex.Variable is TupleExpressionSyntax tupleExpression) {
					if(tupleExpression.Arguments.Count == n.deconstructDatas.Count) {
						for(int i = 0; i < n.deconstructDatas.Count; i++) {
							var arg = tupleExpression.Arguments[i];
							var ddata = n.deconstructDatas[i];
							if(arg.Expression is DeclarationExpressionSyntax declarationExpression) {
								...
								if(designation is SingleVariableDesignationSyntax singleVariableDesignation) {
									ddata.name = singleVariableDesignation.Identifier.ValueText;
								}
								CSharpParser.RegisterSymbol(CSharpParser.GetSymbol(designation, data.model), n, _ => CSharpParser.CreateFromPort(ddata.port));
								CSharpParser.RegisterAutoProxyConnectionForPort(ddata.port);
							}
						}
					}
					else {
				...
				else if(ex.Variable is DeclarationExpressionSyntax expressionSyntax) {
					var designationExpression = expressionSyntax.Designation;
					if(designationExpression is ParenthesizedVariableDesignationSyntax parenthesizedVariable) {
						if(parenthesizedVariable.Variables.Count == n.deconstructDatas.Count) {
							for(int i = 0; i < n.deconstructDatas.Count; i++) {
								var designation = parenthesizedVariable.Variables[i];
								var ddata = n.deconstructDatas[i];
								if(designation is SingleVariableDesignationSyntax singleVariableDesignation) {
									ddata.name = singleVariableDesignation.Identifier.ValueText;
								}
			...
			if((syntax as ExpressionStatementSyntax).Expression is AssignmentExpressionSyntax) {
				...
				if(left is ElementAccessExpressionSyntax && expression.Kind() == SyntaxKind.SimpleAssignmentExpression && data.CanCreateNode) {
					...
					if(arguments.Count == 1) {
						var type = CSharpParser.ParseType(elementAccess.Expression, data.model)?.type;
						if(type != null && type.HasImplementInterface(typeof(IList))) {
				...
				if(left is ElementAccessExpressionSyntax) {
					member = CSharpParser.GetMemberFromExpression(left, new ParserSetting(data, left is ElementAccessExpressionSyntax), out var parameters);
					...
					//	var members = member.GetMembers();
					//	if(members.Length == 0) {
					//		UnityEngine.Debug.Log(member.type);
					//	}
					//}
					result = new ParserResult() {
						node = n,
					...
					if(!CSharpParser.IsSupportInline(mData)) {
						mData = CSharpParser.ToValueNode(mData, null, new ParserSetting(data), n.value.type.IsCastableTo(typeof(System.Delegate)));
					}
		...
		public override bool ExpressionHandler(ExpressionSyntax syntax, ParserData data, out MemberData value) {
			...
			if(syntax is AnonymousMethodExpressionSyntax) {
				...
				if(body != null) {
					...
					else if(body.node.nodeObject.CanGetValue()) {
					...
					if(member != null && member.targetType == MemberData.TargetType.NodePort) {
						...
						if(port is ValueOutput) {
							if(node.returnType != typeof(void)) {
								...
								if(port.hasValidConnections && port.ValidConnections.First().Input is ValueInput vi && vi.node.primaryFlowInput != null) {
									node.body.ConnectTo(vi.node.primaryFlowInput);
								}
								else {
									throw new System.Exception(expression.Body.ToString());
								}
	...
	public class ThrowStatementHandles : SyntaxHandles {
		...
		public override bool ExpressionHandler(ExpressionSyntax syntax, ParserData data, out MemberData value) {
			if(syntax is ThrowExpressionSyntax) {
				var expression = (syntax as ThrowExpressionSyntax).Expression;
				MemberData val;
				if(CSharpParser.TryParseExpression(expression, data.model, data.parent, out val)) {
					var n = CSharpParser.CreateNode<NodeThrow>("throw", data.parent);
					n.value.ConnectTo(val);
					value = CSharpParser.CreateFromPort(n.enter);
					return true;
				}
			}
			return base.ExpressionHandler(syntax, data, out value);
	...
	public class ConditionalAccessExpressionHandles : SyntaxHandles {
		public override bool StatementHandler(StatementSyntax syntax, ParserData data, out ParserResult result) {
			if(syntax is ExpressionStatementSyntax && data.CanCreateNode) {
				...
				if(expression.Expression is ConditionalAccessExpressionSyntax conditional) {
					var ifNode = CSharpParser.CreateNode<NodeIf>("If", data.parent);
					var value = CSharpParser.ParseExpression(conditional.Expression, new ParserSetting(data));
					if(value != null) {
						value = CSharpParser.ToValueNode(value, null, new ParserSetting(data));
					}
					var comparison = CSharpParser.CreateNode<ComparisonNode>("Comparison", data.parent);
					comparison.operatorKind = ComparisonType.NotEqual;
					comparison.inputA.AssignToDefault(value);
					if(comparison.inputA.isConnected) {
						comparison.inputA.connections.First().isProxy = true;
					}
					comparison.inputB.AssignToDefault(MemberData.Null);
					ifNode.condition.ConnectTo(CSharpParser.CreateFromPort(comparison.output));
					var whenNotNull = CSharpParser.ParseExpression(conditional.WhenNotNull, new ParserSetting(data));
					MultipurposeNode whenNotNullNode;
					if(whenNotNull.targetType == MemberData.TargetType.NodePort) {
						whenNotNullNode = (whenNotNull.Get(null) as UPort).GetNode() as MultipurposeNode;
					}
					else {
						whenNotNullNode = CSharpParser.CreateMultipurposeNode("node", data.parent, whenNotNull);
					}
					whenNotNullNode.instance.AssignToDefault(value);
					ifNode.onTrue.ConnectTo(whenNotNullNode.enter);
					result = new ParserResult() {
						node = ifNode,
						next = (nod) => {
							ifNode.exit.ConnectTo(nod.nodeObject.primaryFlowInput);
						}
					};
					return true;
				}
		...
		public override bool ExpressionHandler(ExpressionSyntax syntax, ParserData data, out MemberData value) {
			if(syntax is ConditionalAccessExpressionSyntax && data.CanCreateNode) {
				var conditional = syntax as ConditionalAccessExpressionSyntax;
				var conditionalNode = CSharpParser.CreateNode<ConditionalNode>("conditional", data.parent);
				var cvalue = CSharpParser.ParseExpression(conditional.Expression, new ParserSetting(data));
				if(cvalue != null) {
					cvalue = CSharpParser.ToValueNode(cvalue, null, new ParserSetting(data));
				}
				var comparison = CSharpParser.CreateNode<ComparisonNode>("Comparison", data.parent);
				comparison.operatorKind = ComparisonType.NotEqual;
				comparison.inputA.AssignToDefault(cvalue);
				if(comparison.inputA.isConnected) {
					comparison.inputA.connections.First().isProxy = true;
				}
				comparison.inputB.AssignToDefault(MemberData.Null);
				conditionalNode.condition.ConnectTo(CSharpParser.CreateFromPort(comparison.output));
				var whenNotNull = CSharpParser.ParseExpression(conditional.WhenNotNull, new ParserSetting(data));
				MultipurposeNode whenNotNullNode;
				if(whenNotNull.targetType == MemberData.TargetType.NodePort) {
					whenNotNullNode = (whenNotNull.Get(null) as UPort).GetNode() as MultipurposeNode;
				...
				whenNotNullNode.instance.AssignToDefault(cvalue);
				conditionalNode.onTrue.ConnectTo(whenNotNullNode.output);
				value = CSharpParser.CreateFromPort(conditionalNode.output);
				return true;
			}
	...
	public class MemberBindingExpressionHandles : SyntaxHandles {
		public override bool ExpressionHandler(ExpressionSyntax syntax, ParserData data, out MemberData value) {
			if(syntax is MemberBindingExpressionSyntax && data.CanCreateNode) {
				var expression = syntax as MemberBindingExpressionSyntax;
				value = CSharpParser.ParseExpression(expression.Name, new ParserSetting(data));
				return true;
			}
			return base.ExpressionHandler(syntax, data, out value);
		}
		...
		public override bool StatementHandler(StatementSyntax syntax, ParserData data, out ParserResult result) {
			if(syntax is LocalDeclarationStatementSyntax && data.CanCreateNode) {
				...
				foreach(var dec in expression.Declaration.Variables) {
					...
					node.type = CSharpParser.ParseType(expression.Declaration.Type, data.model);
					node.Register();
					if(expression.Declaration.Type is RefTypeSyntax) {
						node.byRef = true;
					}
	...
	public class ExpressionStatementHandles : SyntaxHandles {
		public override int order =>  int.MaxValue;

		public override bool StatementHandler(StatementSyntax syntax, ParserData data, out ParserResult result) {
			if(syntax is ExpressionStatementSyntax statement) {
				var ex = statement.Expression;
				if(CSharpParser.TryParseExpression(ex, new ParserSetting(data), out var member)) {
					if(member != null && member.isAssigned && member.targetType != MemberData.TargetType.NodePort) {
						var n = CSharpParser.CreateMultipurposeNode("node", data.parent, member);
						result = new ParserResult() {
							node = n
						};
						return true;
	...
	public class TupleExpressionHandles : SyntaxHandles {
		public override bool ExpressionHandler(ExpressionSyntax syntax, ParserData data, out MemberData value) {
			if(syntax is TupleExpressionSyntax) {
				var expression = syntax as TupleExpressionSyntax;
				var symbol = CSharpParser.GetTypeSymbol(expression, data.model);
				var type = CSharpParser.ParseType(symbol);
				if(type != null) {
					if(data.CanCreateNode) {
						var ctor = type.type.GetMember(".ctor").FirstOrDefault() as ConstructorInfo;
						if(ctor != null) {
							var members = new MemberData[expression.Arguments.Count];
							for(int i = 0; i < expression.Arguments.Count; i++) {
								members[i] = CSharpParser.ParseExpression(expression.Arguments[i].Expression, data.model, data.parent);
							}
							var node = CSharpParser.CreateMultipurposeNode("TupleCreation", data.parent, MemberData.CreateFromMember(ctor), members);
							value = CSharpParser.CreateFromPort(node.output);
							return true;
						}
						throw null;
	...
	public class RefExpressionHandles : SyntaxHandles {
		public override bool ExpressionHandler(ExpressionSyntax syntax, ParserData data, out MemberData value) {
			if(syntax is RefExpressionSyntax) {
				var expression = syntax as RefExpressionSyntax;
				value = CSharpParser.ParseExpression(expression.Expression, data.model, data.parent);
				return true;
			}
			return base.ExpressionHandler(syntax, data, out value);
