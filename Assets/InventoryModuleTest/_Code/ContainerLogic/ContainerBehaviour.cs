...
            if (this == null)
            ...
            if (!Application.isPlaying)
            {
                if (SlotPrefab == null)
                    return;
                // Set up or tear down the Viewer hierarchy container first
                GenerateViewerEditor(ShowInEditor);

                if (SlotPrefab != null && UpdateVisuals)
                {
                    GenerateSlots(StartSize);

                }
                else if (SlotParent != null && UpdateVisuals == false)
                {
                    if (!Application.isPlaying)
                    {
